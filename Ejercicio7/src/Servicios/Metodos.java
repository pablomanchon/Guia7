...
public class Metodos {
    ...
    public Persona crearPersona(int numPersona, Persona p1) {
    ...
    public int calcularIMC(int peso, double altura, Persona p1) {
        double pesoIdeal = peso / Math.pow(altura, 2);
        if (pesoIdeal < 20) {
            p1.setIMC("Debajo del peso ideal");
            return -1;
        } else if (pesoIdeal >= 20 && pesoIdeal <= 25) {
            p1.setIMC("Peso ideal");
            return 0;
        } else {
            p1.setIMC("Sobrepeso");
            return 1;
